...
class Model {
  ...
  init() {
    ...
    this.controller.config.deviceControllerList.forEach(deviceControllerInfo => {
      this.deviceClientModel.setDevice(deviceControllerInfo.current.deviceInfo, {idKey: 'target_id', deviceCategoryKey: 'target_category'});
    });
   ...
   */
  onDeviceData(deviceController){
    // BU.CLI('onInverterData');
    // 인버터 컨트롤러의 동작 상태 정보를 가져옴
    let deviceOperationInfo = deviceController.getDeviceOperationInfo();
    // BU.CLIN(deviceOperationInfo);
    // 모델에 데이터 갱신
    let dataStorageContainer = this.deviceClientModel.onDeviceOperationInfo(deviceOperationInfo, deviceController.config.deviceInfo.target_category);
